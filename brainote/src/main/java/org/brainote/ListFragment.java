...
import org.brainote.async.NoteProcessorCategorize;
import org.brainote.async.NoteProcessorTrash;
...
import org.brainote.models.UndoBarController;
import org.brainote.models.adapters.NoteAdapter;
import org.brainote.models.listeners.AbsListViewScrollDetector;
import org.brainote.utils.BitmapHelper;
import org.brainote.utils.Constants;
import org.brainote.utils.KeyboardUtils;
import org.brainote.utils.Navigation;
...
import org.brainote.async.NoteLoaderTask;
import org.brainote.async.NoteProcessorArchive;
import org.brainote.db.DbHelper;
import org.brainote.models.adapters.NavDrawerCategoryAdapter;
import org.brainote.models.listeners.OnNotesLoadedListener;
import org.brainote.models.listeners.OnViewTouchedListener;
import org.brainote.models.views.InterceptorLinearLayout;
...
public class ListFragment extends Fragment implements OnNotesLoadedListener, OnViewTouchedListener,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        return inflater.inflate(org.brainote.R.layout.fragment_list, container, false);
    }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        initTitle();
        ubc = new UndoBarController(getActivity().findViewById(org.brainote.R.id.undobar), this);
    }
    ...
    private void initFab() {
        fab = (FloatingActionsMenu) getActivity().findViewById(org.brainote.R.id.fab);
        ...
        fab.findViewById(org.brainote.R.id.fab_checklist).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                Note note = new Note();
        ...
        fab.findViewById(org.brainote.R.id.fab_camera).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    private void initTitle() {
        String[] navigationList = getResources().getStringArray(org.brainote.R.array.navigation_list);
        String[] navigationListCodes = getResources().getStringArray(org.brainote.R.array.navigation_list_codes);
        ...
        title = title == null ? getString(org.brainote.R.string.title_activity_list) : title;
    ...
    private void initEasterEgg() {
        empyListItem = (TextView) getActivity().findViewById(org.brainote.R.id.empty_list);
    ...
    private void stopJingles() {
        if (jinglesAnimation != null) {
            ...
            empyListItem.setCompoundDrawablesWithIntrinsicBounds(0, org.brainote.R.animator.jingles_animation, 0, 0);
...
//        }
    ...
    private final class ModeCallback implements android.support.v7.view.ActionMode.Callback {
        ...
        public boolean onCreateActionMode(ActionMode mode, Menu menu) {
            ...
            inflater.inflate(org.brainote.R.menu.menu_list, menu);
            actionMode = mode;
        ...
        public void onDestroyActionMode(ActionMode mode) {
            ...
            for (int i = 0; i < listAdapter.getSelectedItems().size(); i++) {
                ...
                if (listAdapter.getCount() > key && listAdapter.getItem(key) != null && v != null) {
                    listAdapter.restoreDrawable(listAdapter.getItem(key), v.findViewById(org.brainote.R.id.card_layout));
                }
            }
        ...
        public boolean onActionItemClicked(final ActionMode mode, final MenuItem item) {
            Integer[] protectedActions = {org.brainote.R.id.menu_select_all, org.brainote.R.id.menu_merge};
    ...
    private void toggleListViewItem(View view, int position) {
        ...
        LinearLayout v = (LinearLayout) view.findViewById(org.brainote.R.id.card_layout);
        if (!getSelectedNotes().contains(note)) {
            getSelectedNotes().add(note);
            listAdapter.addSelectedItem(position);
            v.setBackgroundColor(getResources().getColor(org.brainote.R.color.list_bg_selected));
        } else {
    ...
    private void initListView() {
        list = (DynamicListView) getActivity().findViewById(org.brainote.R.id.list);
        ...
        ((InterceptorLinearLayout) getActivity().findViewById(org.brainote.R.id.list_root))
                .setOnViewTouchedListener(this);
    }


    private void zoomListItem(final View view, final Note note) {
        final long animationDuration = getResources().getInteger(org.brainote.R.integer.zooming_view_anim_time);
        ...
        getActivity().findViewById(org.brainote.R.id.list_root)
    ...
    private ImageView getZoomListItemView(View view, Note note) {
        final ImageView expandedImageView = (ImageView) getActivity().findViewById(org.brainote.R.id.expanded_image);
        View targetView = null;
        if (note.getAttachmentsList().size() > 0) {
            targetView = view.findViewById(org.brainote.R.id.attachmentThumbnail);
        }
        if (targetView == null && note.getCategory() != null) {
            targetView = view.findViewById(org.brainote.R.id.category_marker);
        }
        if (targetView == null) {
...
//		initShowCase();
    ...
    private void initSortingSubmenu() {
        final String[] arrayDb = getResources().getStringArray(org.brainote.R.array.sortable_columns);
        final String[] arrayDialog = getResources().getStringArray(org.brainote.R.array.sortable_columns_human_readable);
        int selected = Arrays.asList(arrayDb).indexOf(prefs.getString(Constants.PREF_SORTING_COLUMN, arrayDb[0]));

        SubMenu sortMenu = this.menu.findItem(org.brainote.R.id.menu_sort).getSubMenu();
    ...
    private void prepareActionModeMenu() {
        ...
        if (navigation == Navigation.TRASH) {
            ...
            if (getSelectedCount() == 1) {
                menu.findItem(org.brainote.R.id.menu_share).setVisible(true);
                menu.findItem(org.brainote.R.id.menu_merge).setVisible(false);
                menu.findItem(org.brainote.R.id.menu_archive)
                        .setVisible(showArchive && !getSelectedNotes().get(0).isArchived
                                ());
                menu.findItem(org.brainote.R.id.menu_unarchive)
                        .setVisible(showUnarchive && getSelectedNotes().get(0).isArchived
                                ());
            } else {
                menu.findItem(org.brainote.R.id.menu_share).setVisible(false);
                menu.findItem(org.brainote.R.id.menu_merge).setVisible(true);
                menu.findItem(org.brainote.R.id.menu_archive).setVisible(showArchive);
                menu.findItem(org.brainote.R.id.menu_unarchive).setVisible(showUnarchive);

            }
            menu.findItem(org.brainote.R.id.menu_category).setVisible(true);
            menu.findItem(org.brainote.R.id.menu_tags).setVisible(true);
            menu.findItem(org.brainote.R.id.menu_trash).setVisible(true);
        }
        menu.findItem(org.brainote.R.id.menu_select_all).setVisible(true);
    ...
    private void initSearchView(final Menu menu) {
        ...
        searchMenuItem = menu.findItem(org.brainote.R.id.menu_search);

        // Associate searchable configuration with the SearchView
        SearchManager searchManager = (SearchManager) getActivity().getSystemService(Context.SEARCH_SERVICE);
        searchView = (SearchView) MenuItemCompat.getActionView(menu.findItem(org.brainote.R.id.menu_search));
        ...
        MenuItemCompat.setOnActionExpandListener(searchMenuItem, new MenuItemCompat.OnActionExpandListener() {
            ...
            public boolean onMenuItemActionCollapse(MenuItem item) {
                ...
                // collapsed
                searchQuery = null;
                if (getActivity().findViewById(org.brainote.R.id.search_layout).getVisibility() == View.VISIBLE) {
            ...
            public boolean onMenuItemActionExpand(MenuItem item) {
                searchView.setOnQueryTextListener(new OnQueryTextListener() {
                    ...
                    public boolean onQueryTextChange(String pattern) {
                        View searchLayout = getActivity().findViewById(org.brainote.R.id.search_layout);
    ...
    private void setActionItemsVisibility(Menu menu, boolean searchViewHasFocus) {
        ...
        menu.findItem(org.brainote.R.id.menu_search).setVisible(!drawerOpen);
        menu.findItem(org.brainote.R.id.menu_filter).setVisible(!drawerOpen && !filterPastReminders && navigationReminders &&
                !searchViewHasFocus);
        menu.findItem(org.brainote.R.id.menu_filter_remove).setVisible(!drawerOpen && filterPastReminders && navigationReminders
                && !searchViewHasFocus);
        menu.findItem(org.brainote.R.id.menu_sort).setVisible(!drawerOpen && !navigationReminders && !searchViewHasFocus);
        menu.findItem(org.brainote.R.id.menu_expanded_view).setVisible(!drawerOpen && !expandedView && !searchViewHasFocus);
        menu.findItem(org.brainote.R.id.menu_contracted_view).setVisible(!drawerOpen && expandedView && !searchViewHasFocus);
        menu.findItem(org.brainote.R.id.menu_empty_trash).setVisible(!drawerOpen && navigationTrash);
        menu.findItem(org.brainote.R.id.menu_tags).setVisible(searchViewHasFocus);
    }


    @Override
    public boolean onOptionsItemSelected(final MenuItem item) {
        Integer[] protectedActions = {org.brainote.R.id.menu_empty_trash};
        if (Arrays.asList(protectedActions).contains(item.getItemId())) {
        ...
        if (actionMode == null) {
            switch (item.getItemId()) {
                case android.R.id.home:
                    ...
                    break;
                case org.brainote.R.id.menu_filter:
                    filterReminders(true);
                    break;
                case org.brainote.R.id.menu_filter_remove:
                    filterReminders(false);
                    break;
                case org.brainote.R.id.menu_tags:
                    filterByTags();
                    break;
                case org.brainote.R.id.menu_sort:
                    initSortingSubmenu();
                    break;
                case org.brainote.R.id.menu_expanded_view:
                    switchNotesView();
                    break;
                case org.brainote.R.id.menu_contracted_view:
                    switchNotesView();
                    break;
                case org.brainote.R.id.menu_empty_trash:
                    emptyTrash();
                    break;
            }
        } else {
            switch (item.getItemId()) {
                ...
                case org.brainote.R.id.menu_share:
                    share();
                    break;
                case org.brainote.R.id.menu_merge:
                    merge();
                    break;
                ...
                case org.brainote.R.id.menu_delete:
                    deleteNotes();
                    break;
                case org.brainote.R.id.menu_select_all:
                    selectAllNotes();
                    break;
...
//                    break;
            ...
            case REQUEST_CODE_CATEGORY:
                ...
                switch (resultCode) {
                    ...
                    case Activity.RESULT_FIRST_USER:
                        getMainActivity().showMessage(org.brainote.R.string.category_deleted, ONStyle.ALERT);
                        break;
                    default:
                        break;
                }
    ...
    private void emptyTrash() {
        new MaterialDialog.Builder(getActivity())
                .content(org.brainote.R.string.empty_trash_confirmation)
    ...
    public void toggleSearchLabel(boolean activate) {
        View searchLabel = getActivity().findViewById(org.brainote.R.id.search_layout);
        boolean isActive = searchLabel.getVisibility() == View.VISIBLE;
        if (activate) {
            ((android.widget.TextView) getActivity().findViewById(org.brainote.R.id.search_query)).setText(Html.fromHtml("<i>"
                    + getString(org.brainote.R.string.search) + ":</i> " + searchQuery));
            searchLabel.setVisibility(View.VISIBLE);
            getActivity().findViewById(org.brainote.R.id.search_cancel).setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    toggleSearchLabel(false);
                }
            ...
            if (isActive) {
                getActivity().findViewById(org.brainote.R.id.search_layout).setVisibility(View.GONE);
                searchTags = null;
                searchQuery = null;
    ...
    public void onNotesLoaded(ArrayList<Note> notes) {
        layoutSelected = prefs.getBoolean(Constants.PREF_EXPANDED_VIEW, true) ? org.brainote.R.layout.note_layout_expanded
                : org.brainote.R.layout.note_layout;
        ...
        if (notes.size() == 0) list.setEmptyView(getActivity().findViewById(org.brainote.R.id.empty_list));
        ...
        animate(list).setDuration(getResources().getInteger(org.brainote.R.integer.list_view_fade_anim)).alpha(1);
    }
    ...
    public void trashNotes(boolean trash) {
        ...
        if (listAdapter.getCount() == 0)
            list.setEmptyView(getActivity().findViewById(org.brainote.R.id.empty_list));

        finishActionMode();

        // Advice to user
        if (trash) {
            getMainActivity().showMessage(org.brainote.R.string.note_trashed, ONStyle.WARN);
        } else {
            getMainActivity().showMessage(org.brainote.R.string.note_untrashed, ONStyle.INFO);
        }

        // Creation of undo bar
        if (trash) {
            ubc.showUndoBar(false, selectedNotesSize + " " + getString(org.brainote.R.string.trashed), null);
            hideFab();
            undoTrash = true;
        } else {
    ...
    private void selectAllNotes() {
        for (int i = 0; i < list.getChildCount(); i++) {
            LinearLayout v = (LinearLayout) list.getChildAt(i).findViewById(org.brainote.R.id.card_layout);
    ...
    public void archiveNotes(boolean archive) {
        ...
        // If list is empty again Mr Jingles will appear again
        if (listAdapter.getCount() == 0) list.setEmptyView(getActivity().findViewById(org.brainote.R.id.empty_list));

        // Advice to user
        int msg = archive ? org.brainote.R.string.note_archived : org.brainote.R.string.note_unarchived;
        Style style = archive ? ONStyle.WARN : ONStyle.INFO;
        getMainActivity().showMessage(msg, style);

        // Creation of undo bar
        if (archive) {
            ubc.showUndoBar(false, selectedNotesSize + " " + getString(org.brainote.R.string.archived), null);
            hideFab();
            undoArchive = true;
        } else {
            getSelectedNotes().clear();
    ...
    private void categorizeNotesExecute(Category category) {
        ...
        String msg;
        if (category != null) {
            msg = getResources().getText(org.brainote.R.string.notes_categorized_as) + " '" + category.getName() + "'";
        } else {
            msg = getResources().getText(org.brainote.R.string.notes_category_removed).toString();
        }
        getMainActivity().showMessage(msg, ONStyle.INFO);

        // Creation of undo bar
        if (category == null) {
            ubc.showUndoBar(false, getString(org.brainote.R.string.notes_category_removed), null);
            hideFab();
            undoCategorize = true;
    ...
    private void tagNotesExecute(List<Tag> tags, Integer[] selectedTags, Integer[] preSelectedTags) {
        ...
        getMainActivity().showMessage(org.brainote.R.string.tags_added, ONStyle.INFO);
    }
